...
## there are three types of pdfs, requiring different greps to subset the tables correctly
## 2006:2011 <- old style, very unstructured
## 2012:2017 <- more structured
## 2018+ <- the easiest
## TODO: fix dates to be YMD
## tests: 2012 works
## tests: 2008 works
## tests: 2018 works 
## tests: may2010 fails
## tests: oct2010 fails
## tests: oct2009 fails
# pdf <- list.files("appendix_f", pattern = "*.pdf", full.names = T)
get_tables <- function(pdf) {
  require(lubridate)
  require(dplyr)
  require(pdftools)
  require(stringr)
